const MEMBERS_DB = MRAACommon.connect(MEMBERSHIP_ID)
const M_TABLES = {
...
function getStatusList() {
   let list = M_SETTINGS_TABLE.getRange(
      M_SETTINGS_FIELDS_MAP["status-list"] + M_SETTINGS_TABLE.getLastRow()
   ).getDisplayValues()
...
function getMembershipTypeList() {
   let list = M_SETTINGS_TABLE.getRange(
      M_SETTINGS_FIELDS_MAP["membership-type-list"] +
         M_SETTINGS_TABLE.getLastRow()
   ).getDisplayValues()
   list = list.map((l) => l[0]) // convert from [][] to []
   return list.filter((l) => l !== "") // remove empty array elements
}
...
function getTotalMembers(type) {
   ...
   switch (type) {
      case "all":
         count = M_DASHBOARD_TABLE.getRange(
            M_DASHBOARD_FIELDS_MAP["counts"]["members"]
         ).getDisplayValue()
      ...
      case "pending":
         count = M_DASHBOARD_TABLE.getRange(
            M_DASHBOARD_FIELDS_MAP["counts"]["pending-members"]
         ).getDisplayValue()
         count = parseInt(count)
         break

      case "exhibiting":
         var list = M_DASHBOARD_TABLE.getRange(
            M_DASHBOARD_FIELDS_MAP["counts"]["membership-type"]
         ).getDisplayValues()
...
function getDuesPaid() {
   let d = M_DASHBOARD_TABLE.getRange(
      M_DASHBOARD_FIELDS_MAP["counts"]["dues-paid"]
   ).getDisplayValue()
